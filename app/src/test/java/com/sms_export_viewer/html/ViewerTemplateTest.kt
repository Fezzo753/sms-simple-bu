package com.sms_export_viewer.html
...
class ViewerTemplateTest {
    ...
    @Test fun `template version sentinel is present and bumped`() {
        ...
        assertThat(template).contains("viewer-template-version: 5")
    }

    @Test fun `print stylesheet undoes the screen overflow clipping`() {
        // The global `html, body { height: 100%; overflow: hidden }` rule has to be
        // explicitly overridden inside @media print. Without these the WebView measures
        // body as viewport-height when handing pages to PrintManager and the PDF only
        // contains the events that happened to fit on screen.
        val printBlock = printMediaQueryBlock()
        assertThat(printBlock).contains("html, body")
        assertThat(printBlock).contains("height: auto !important")
        assertThat(printBlock).contains("overflow: visible !important")
    }

    private fun printMediaQueryBlock(): String {
        val start = template.indexOf("@media print")
        check(start >= 0) { "no @media print rule in template" }
        var depth = 0
