 ...
 * so the page can't print or save files on its own. We expose two endpoints that the
 * page calls when it detects the bridge (and falls back to the browser-native APIs
 * otherwise — so the same HTML works on a desktop browser).
 *
 * Methods are invoked from a background thread by the WebView; UI work is hopped to
